...
// NOTE: this used to write every downloaded media buffer to a disk file
// named e.g. "undefined.jpg" (in the process's working directory) and then
// immediately read that same file back with a *blocking* fs.readFileSync —
// two unnecessary synchronous disk round-trips per download, on top of
// blocking the whole Node event loop (freezing every other user's session
// and every other command) while the write/read happened. Worse: because
// the filename wasn't unique per call, two users downloading media at
// close to the same time could overwrite each other's temp file and end up
// with the wrong media. None of that disk I/O was ever needed — the buffer
// already holds the full file in memory, so we just return it directly.
const downloadMediaMessage = async (m, filename) => {
    ...
    const dlType = typeMap[m.type]
    if (!dlType) return null

    const stream = await downloadContentFromMessage(m.msg, dlType)
    const chunks = []
