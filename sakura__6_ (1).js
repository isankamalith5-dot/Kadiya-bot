...
// Load all plugin commands from cmd/<category>/*.js once at startup.
// Drop new files in there any time — no other code needs to change.
...
// Cached latest WA Web protocol version. Fetched lazily and reused so we
// don't hit WhatsApp's version-check endpoint on every socket creation.
// See getWaVersion() usage in EmpirePair() and validateAndCleanSessions().
let cachedWaVersion = null;
let cachedWaVersionAt = 0;
async function getWaVersion() {
  const ONE_HOUR = 60 * 60 * 1000;
  if (cachedWaVersion && (Date.now() - cachedWaVersionAt) < ONE_HOUR) return cachedWaVersion;
  try {
    const { version } = await fetchLatestBaileysVersion();
    cachedWaVersion = version;
    cachedWaVersionAt = Date.now();
  } catch (e) { console.warn('fetchLatestBaileysVersion failed, using bundled version', e?.message || e); }
...
// Tracks connection state ourselves instead of relying on
// mongoClient.topology.isConnected() — that check is unreliable/removed on
// modern mongodb driver versions (this project uses ^6.9.0), which meant
// initMongo() was silently reconnecting to Mongo from scratch (new TCP
// handshake + auth + recreating 5 indexes) on almost every single command,
// since loadUserConfigFromMongo() calls initMongo() on every incoming
// message. That was the main cause of slow command replies.
let mongoConnected = false;
let mongoConnectingPromise = null;

async function initMongo() {
  if (mongoConnected && mongoClient) return;

  // If a connect is already in flight (e.g. several messages arrived at
  // once right after startup), reuse that same attempt instead of racing
  // multiple new MongoClients into existence.
  ...
  mongoConnectingPromise = (async () => {
    ...
    mongoClient.on('close', () => { mongoConnected = false; });
    mongoClient.on('error', () => { mongoConnected = false; });
  })();
...
// loadUserConfigFromMongo() runs on EVERY incoming command (menu/alive/ping
// included) purely for the permission check + botName/logo lookup — that's
// a full network round-trip to Mongo before the bot can even start typing
// a reply, which is the main reason those "instant" commands still felt
// laggy after the logo/connection fixes. Config docs change rarely (only
// via /settings or delsession), so cache them per-number for a short TTL
// and keep the cache in sync on every write.
const userConfigCache = new Map(); // key: sanitized number -> { config, ts }
const USER_CONFIG_CACHE_TTL_MS = 20 * 1000; // 20s
...
async function setUserConfigInMongo(number, conf) {
  try {
    ...
    userConfigCache.set(sanitized, { config: conf, ts: Date.now() }); // keep cache in sync instead of just invalidating
...
async function loadUserConfigFromMongo(number) {
  ...
  if (cached && (Date.now() - cached.ts) < USER_CONFIG_CACHE_TTL_MS) return cached.config;
  ...
  try {
    ...
    // serve stale cache rather than nothing if Mongo hiccups
...
// Frequently-used commands (menu/alive/ping/etc.) send the bot's logo image
// on every single call. Without caching, that meant re-downloading the same
// remote image (or re-reading the same local file) from scratch on every
// command, which is the main reason those "instant" status commands felt
// slow. This caches the raw buffer in memory for a while so repeat calls
// reuse it instead of hitting the network/disk again.
const logoBufferCache = new Map(); // key: logo path/url -> { buffer, ts }
const LOGO_CACHE_TTL_MS = 30 * 60 * 1000; // 30 minutes

async function getLogoBuffer(logo) {
  const key = String(logo || '');
  const cached = logoBufferCache.get(key);
  if (cached && (Date.now() - cached.ts) < LOGO_CACHE_TTL_MS) return cached.buffer;

  let buffer;
  if (key.startsWith('http')) {
    const res = await axios.get(key, { responseType: 'arraybuffer', timeout: 8000 });
    buffer = Buffer.from(res.data);
  } else {
    buffer = await fs.readFile(key);
  }

  logoBufferCache.set(key, { buffer, ts: Date.now() });
  return buffer;
}

// Convenience wrapper for the extremely common "send this caption with the
// bot's logo, quoted, with channel context, falling back to plain text if
// the image fails" pattern used by alive/ping/menu/etc. Sending a cached
// Buffer (instead of {url: ...}) skips Baileys having to fetch the remote
// image itself on every call.
async function sendLogoCaption(socket, sender, logo, caption, contextInfo, quoted) {
  try {
    const buf = await getLogoBuffer(logo);
    return await socket.sendMessage(sender, { image: buf, caption, contextInfo }, quoted ? { quoted } : undefined);
  } catch (e) {
    try {
      return await socket.sendMessage(sender, { text: caption, contextInfo }, quoted ? { quoted } : undefined);
    } catch (e2) { console.error('sendLogoCaption failed entirely:', e2?.message || e2); }
  }
}

const activeSockets = new Map();
// In-memory guard so the "connected" welcome message isn't resent every time
// EmpirePair() re-runs for a number that's already been welcomed (e.g. on
// reconnects after a network blip). Backed up by a persisted flag in Mongo
// (userConfig.welcomeSent) so it also survives process restarts.
const welcomeAnnouncedNumbers = new Set();
...
function setupCommandHandlers(socket, number) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    const __cmdStart = Date.now(); // used by alive/ping/menu to log total handling time — see [TIMING] logs
  ...
  // Reaction is just decoration — fire it without blocking the actual reply.
  socket.sendMessage(sender, { react: { text: '🐽', key: msg.key } }).catch(()=>{});

  await sendLogoCaption(socket, sender, logo, aliveCaption, channelContext, msg);
  console.log(`[TIMING] alive handled in ${Date.now() - __cmdStart}ms`);
  break;
}
...
case 'ping': {
  ...
  await sendLogoCaption(socket, sender, logo, pingCaption, channelContext, msg);
  console.log(`[TIMING] ping handled in ${Date.now() - __cmdStart}ms`);
  break;
}
...
　　˚₊‧꒰ა 🌸 ໒꒱‧₊˚`
  ...
  try {
    ...
    // FIX (root cause of "reply comes ~5 min later"): the watermark tag
    // fetch previously had NO timeout at all and ran AFTER the main mp3
    // finished downloading. If translate.google.com was slow/unreachable,
    // axios would hang with no error until the OS socket eventually gave up
    // — often several minutes. It's now capped at 8s and, since it doesn't
    // depend on the main download, runs alongside it instead of after it —
    // roughly halving the wait for this command.
    const [dlResp, tagResp] = await Promise.all([
      axios.get(sDownloadUrl, {
        responseType: 'stream',
        timeout: 120000,
        headers: { 'User-Agent': 'Mozilla/5.0' }
      }).catch(() => null),
      axios.get(sTagUrl, { responseType: 'stream', timeout: 8000 }).catch(() => null)
    ]);
    ...
    const writeStream = (readable, destPath) => new Promise((resolve, reject) => {
    ...
    const writes = [writeStream(dlResp.data, tmpMp3)];
    if (tagResp?.data) {
      writes.push(writeStream(tagResp.data, tmpTag).catch(() => {}));
    }
...
case 'tiktok': {
  ...
  const tiktokListener = async (msgUpdate) => {
    ...
    socket.sendMessage(sender, { react: { text: '⬇️', key: reply2.key } }).catch(()=>{});
...
case 'ig': {
  ...
  const numberListener = async (update) => {
    try {
      ...
      socket.sendMessage(sender, { react: { text: '✅', key: newMsg.key } }).catch(()=>{});
...
case 'allmenu': {
  ...
  const menuMsg = await sendLogoCaption(socket, sender, logo, menuCaption, channelContext, msg);
  console.log(`[TIMING] menu handled in ${Date.now() - __cmdStart}ms`);
  ...
  const subMenus = {
    ...
    '3': {
      ...
      body:
        ...
        `❍ *${prefix}pair <number>* ┊ Pair a new number\n` +
        `❍ *${prefix}delsession <number>* ┊ Delete a session`
    },
    '4': {
      title: '🌙 ᴏᴛʜᴇʀ ᴍᴇɴᴜ',
      body:
  ...
  // Auto-append any commands dropped into cmd/<category>/*.js. The
  // hardcoded lines above are untouched — new plugin commands just get
  // added on to the end of the matching category automatically.
  const subMenuCategoryMap = { '1': 'main', '2': 'download', '3': 'owner', '4': 'other' };
  for (const [num, category] of Object.entries(subMenuCategoryMap)) {
    const pluginLines = cmdLoader.getCategoryLines(category, prefix);
  ...
  const menuListener = async (msgUpdate) => {
    ...
    await sendLogoCaption(socket, sender, logo, subCaption, channelContext, reply2);
  };
...
case 'pair': {
  const effectiveOwnerPair = (config.OWNER_NUMBER || '').replace(/[^0-9]/g, '');
  const adminsPair = await loadAdminsFromMongo().catch(() => []);
  const isAuthorizedPair = senderNumber === effectiveOwnerPair || adminsPair.includes(senderNumber) || adminsPair.includes(sender);

  if (!isAuthorizedPair) {
    await reply('❌ Only the bot owner or an admin can pair a new number.');
    break;
  }

  const targetPairNumber = (args[0] || '').replace(/[^0-9]/g, '');
  if (!targetPairNumber) {
    await reply(`❌ Usage: *${prefix}pair <number>*\nExample: *${prefix}pair 947xxxxxxxx*`);
    break;
  }

  if (activeSockets.has(targetPairNumber)) {
    await reply(`⚠️ *${targetPairNumber}* is already an active session.`);
    break;
  }

  await reply(`⏳ Requesting pairing code for *${targetPairNumber}*...`);

  const mockPairRes = {
    headersSent: false,
    send: async (data) => {
      mockPairRes.headersSent = true;
      if (data && data.code) {
        await reply(`✅ *Pairing code for ${targetPairNumber}:*\n\n🔑 \`${data.code}\`\n\nOpen WhatsApp → Linked Devices → Link with phone number, and enter this code.`);
      } else if (data && data.error) {
        await reply(`❌ Pairing failed: ${data.error}`);
      }
    },
    status: (code) => ({ send: (data) => mockPairRes.send({ ...data, error: data?.error || `HTTP ${code}` }) })
  };

  try {
    await EmpirePair(targetPairNumber, mockPairRes);
  } catch (e) {
    console.error('pair command error:', e);
    await reply(`❌ Failed to start pairing: ${e.message || e}`);
  }
...
case 'deletesession':
case 'delsesi': {
  const effectiveOwnerDel = (config.OWNER_NUMBER || '').replace(/[^0-9]/g, '');
  const adminsDel = await loadAdminsFromMongo().catch(() => []);
  const isAuthorizedDel = senderNumber === effectiveOwnerDel || adminsDel.includes(senderNumber) || adminsDel.includes(sender);

  if (!isAuthorizedDel) {
    await reply('❌ Only the bot owner or an admin can delete a session.');
    break;
  }

  const sanitizedSelfDel = (number || '').replace(/[^0-9]/g, '');
  const targetDelNumber = args[0] ? args[0].replace(/[^0-9]/g, '') : sanitizedSelfDel;

  if (!targetDelNumber) {
    await reply(`❌ Usage: *${prefix}delsession <number>*\nExample: *${prefix}delsession 947xxxxxxxx*\n(Leave blank to delete this session.)`);
    break;
  }

  await reply(`⏳ Removing session *${targetDelNumber}*...`);

  try {
    const runningDel = activeSockets.get(targetDelNumber);
    if (runningDel) {
      try { if (typeof runningDel.logout === 'function') await runningDel.logout().catch(() => {}); } catch (e) {}
      try { runningDel.ws?.close(); } catch (e) {}
    }
    await deleteSessionAndCleanup(targetDelNumber, runningDel || socket);
    await reply(`✅ Session *${targetDelNumber}* has been removed.`);
  } catch (e) {
    console.error('delsession command error:', e);
    await reply(`❌ Failed to remove session: ${e.message || e}`);
  }
...
case 'set': {
  ...
  break;
      ...
      default: {
        const pluginCmd = cmdLoader.getCommand(command);
        if (pluginCmd) {
          if (pluginCmd.ownerOnly) {
            const sanitizedForOwner = (number || '').replace(/[^0-9]/g, '');
            const effectiveOwnerNumber = (sessionConfig.ownerNumber || config.OWNER_NUMBER || '').replace(/[^0-9]/g, '');
            if (senderNumber !== effectiveOwnerNumber) {
              await reply('❌ Permission denied. This command is owner-only.');
              break;
            }
          }
          try {
            await pluginCmd.execute({
              socket, msg, sender, number, command, args, q, prefix,
              reply, sessionConfig, isGroup, senderNumber
            });
          } catch (pluginErr) {
            console.error(`[cmdLoader] Plugin command "${command}" threw:`, pluginErr);
            await reply(`❌ Error running *${prefix}${command}*: ${pluginErr.message || pluginErr}`);
          }
        }
        break;
      }
...
function setupMessageHandlers(socket) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // Fire-and-forget: these are cosmetic presence pings (mark "unavailable",
    // optionally show a recording indicator). Awaiting them here forced every
    // single incoming message — commands included — to wait on an extra
    // WhatsApp round-trip before the actual command handler even started
    // processing, on the same socket/connection used to send the real reply.
...
async function deleteSessionAndCleanup(number, socketInstance) {
  ...
  try {
    ...
    welcomeAnnouncedNumbers.delete(sanitized);
...
async function EmpirePair(number, res) {
  ...
  // Without this, makeWASocket falls back to the WA Web version baked into
  // whatever baileys version is installed. WhatsApp periodically stops
  // accepting old versions for new logins — the pairing code still gets
  // generated and accepted on the phone, but the socket never actually
  // finishes logging in (it just sits there / keeps reconnecting). Fetching
  // the current version each time fixes that.
  ...
  try {
    ...
    // Auto-expire the pairing code 3 minutes after it's issued. If the user
    // hasn't entered it on their phone by then (socket never finishes
    // registering), we kill this socket/session so a stale code can't be
    // used later — the user has to request a fresh pairing code instead.
    ...
    if (!socket.authState.creds.registered) {
      ...
      if (code) {
        pairingExpiryTimer = setTimeout(async () => {
          if (socket.authState.creds.registered) return; // already paired, nothing to expire
          console.log(`⏰ Pairing code for ${sanitizedNumber} expired (3 min elapsed without pairing). Closing socket — a new code will be needed.`);
    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        ...
        try {
          ...
          // Only send the "connected" welcome messages the very first time
          // this number gets paired. Later reconnects (network drops,
          // restarts, etc.) reuse the same session and should stay silent.
          const alreadyWelcomed = welcomeAnnouncedNumbers.has(sanitizedNumber) || userConfig.welcomeSent === true;
          if (alreadyWelcomed) {
            welcomeAnnouncedNumbers.add(sanitizedNumber);
            return;
          }
          welcomeAnnouncedNumbers.add(sanitizedNumber);
          ...
          try {
            if (String(useLogo).startsWith('http')) {
              ...
              try {
                const buf = await fs.readFile(useLogo);
          ...
          try {
            await setUserConfigInMongo(sanitizedNumber, { ...userConfig, welcomeSent: true });
          } catch (e) { console.error('Failed to persist welcomeSent flag:', e); }
...
async function validateAndCleanSessions() {
  ...
  for (const number of numbers) {
    ...
    try {
      ...
      await fs.writeFile(path.join(checkPath, 'creds.json'), JSON.stringify(mongoDoc.creds, null, 2));
      if (mongoDoc.keys) await fs.writeFile(path.join(checkPath, 'keys.json'), JSON.stringify(mongoDoc.keys, null, 2));

      const { state } = await useMultiFileAuthState(checkPath);
      const logger = pino({ level: 'fatal' });
      const waVersionForCheck = await getWaVersion();
      ...
      const loggedOut = await new Promise((resolve) => {
        ...
        try {
          testSocket = makeWASocket({
            ...
            logger,
            browser: Browsers.macOS('Safari'),
            ...(waVersionForCheck ? { version: waVersionForCheck } : {})
          });
